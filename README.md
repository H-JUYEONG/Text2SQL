...
# (선택) 관리자 권한용 API 키 - 스키마/테이블 명세 조회 허용
# 일반 유저는 데이터 조회만 가능, 스키마/테이블/컬럼 구조 질문은 차단됩니다.
ADMIN_API_KEY="admin_permission"
EOF
```

### 관리자 권한 (임시, 로그인 없이)

- **목표**: 로그인 기능 없이도, **관리자만** 테이블/컬럼 명세(스키마)와 테이블 목록을 조회할 수 있게 함
- **방식**: 
  - 서버: `.env`의 `ADMIN_API_KEY` 값과 동일한 키를 가진 요청만 **관리자**로 인식
  - 클라이언트(웹): 상단 우측 `관리자 키 설정` 버튼으로 키를 한 번만 입력하면, 이후 요청에 `X-ADMIN-KEY` 헤더가 자동으로 포함됨 

- **동작 요약**
  - 일반 유저(ADMIN: OFF)
    - `"테이블 목록 알려줘"`, `"orders 컬럼 뭐 있어?"`, `"스키마/ERD 보여줘"` 같은 **스키마/보안 관련 질문은 서버에서 차단**
    - `"미배송 주문 목록 보여줘"`, `"기사별 배송 건수"` 등 **데이터 조회/분석 질문은 정상 응답**
  - 관리자(ADMIN: ON)
    - `.env`의 `ADMIN_API_KEY`와 같은 값을 `관리자 키 설정`에 입력한 후,
    - 테이블/컬럼 명세, 스키마 관련 질문까지 조회 가능

- **보안 처리 포인트**
  - 스키마/테이블/컬럼 구조/DDL/`information_schema` 조회는 **관리자 키가 없으면 항상 거절**
  - 키는 **헤더(`X-ADMIN-KEY`)로만 전송되며**, 채팅 내용에는 포함되지 않도록 처리
  - 나중에 로그인/역할 기반 권한(SSO/JWT 등)으로 교체하기 쉬운 임시 메커니즘
